...
self.addEventListener("fetch", (event) => {
  ...
  if (
    ...
    // if the assets includes the requested page, we use the cache only method, if not, we just fetch the resource...
    ...
    event.respondWith(fetchReq(event.request, null, /*returnOffline= */ true));
    return;
  }
  ...
  return fetch(req)
    ...
    .catch(async () => {
      ...
      if (returnOffline) {
        ...
        errResponse = `<body style='background:white;display:flex;justify-content:center;align-items:center;padding-block:1rem;'><h1 style='color:black;'>${errMsg}</h1></body>`;
      }
